...
class TSDF:
    ...
    def parse_frame2(self, depth, color, extrinsic):
        if not self.init:
            self.init = True
            self.init_extrinsic_inv = np.linalg.inv(extrinsic)
            self.mean_depth = 2
            self.vol_start = np.array([-1.5, -1.5, 0.5])
            self.vol_end = np.array([1.5, 1.5, 3.5])
            ...
            self.parse_frame2(depth, color, extrinsic)
        else:
            ...
            self.tsdf_diff[mask & weight_mask] = (self.tsdf_diff[mask & weight_mask] * self.tsdf_wt[
                mask & weight_mask] + weight * diff[mask & weight_mask]) / (self.tsdf_wt[mask & weight_mask] + weight)
            self.tsdf_color[mask & weight_mask] = (self.tsdf_color[mask & weight_mask] * np.expand_dims(
                self.tsdf_wt[mask & weight_mask], -1) + weight * color[idx][mask & weight_mask]) \
